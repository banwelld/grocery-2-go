...
export function createProductController({ setProducts, concurrencyControls }) {
  const fetchProducts = () =>
    runExclusive({
      doFetch: () =>
        getData(API_ENDPOINT.PRODUCTS)
          .then((data) => {
            const sortedProducts = data.map(toClient).sort(compareSortValues({ key: 'name' }));
  ...
  const addProduct = (data) => {
    ...
    return runExclusive({
      doFetch: () =>
        postData(API_ENDPOINT.PRODUCTS, payload)
          .then((serverData) => {
            const newProduct = toClient(serverData);
            setProducts((prev) => {
              const updatedProducts = [...prev.filter((p) => p.id !== -1), newProduct];
  ...
  const updateProduct = (data, id) => {
    ...
    setProducts((prev) => {
      ...
      const updatedProducts = prev.map((p) => (p.id === id ? { ...p, ...data } : p));
      return [...updatedProducts].sort(compareSortValues({ key: 'name' }));
    });
    ...
    return runExclusive({
      doFetch: () =>
        patchData(API_ENDPOINT.PRODUCT_ID(id), payload)
          .then((serverData) => {
            const updatedProduct = toClient(serverData);
            setProducts((prev) => {
              const nextProducts = prev.map((p) => (p.id === id ? updatedProduct : p));
              return [...nextProducts].sort(compareSortValues({ key: 'name' }));
            });
          ...
          .catch((err) => {
            ...
            if (originalProduct) {
              setProducts((prev) => {
                const rolledBack = prev.map((p) => (p.id === id ? originalProduct : p));
  ...
  const deleteProduct = (id) => {
    ...
    return runExclusive({
      doFetch: () =>
        deleteData(API_ENDPOINT.PRODUCT_ID(id))
          .then(() => true)
          .catch((err) => {
            ...
            if (originalProduct) {
              setProducts((prev) => {
                ...
                return restoredProducts.sort(compareSortValues({ key: 'name' }));
              });
            }
            throw err;
          }),
