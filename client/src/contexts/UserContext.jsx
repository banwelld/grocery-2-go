...
export function UserProvider({ children }) {
  ...
  const { loadSession, userAuth, userActions } = useMemo(() => {
    const concurrencyControls = {
      lockRef: isBusyRef,
      setPending: setIsPending
    };
    return createUserController({ setUser, setSessionLoaded, navigate, concurrencyControls, userRef: lastUserRef });
  }, [navigate]);
  ...
  const ctx = useMemo(() => {
    return { user, isLoggedIn: !!user, userAuth, userActions, isPending, sessionLoaded };
